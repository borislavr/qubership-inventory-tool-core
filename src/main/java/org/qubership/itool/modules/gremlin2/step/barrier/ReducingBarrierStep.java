...
public abstract class ReducingBarrierStep<S, E> extends AbstractStep<S, E> {
    ...
    protected void processAllPreviousTraversers(List<Traverser<S>> previousTraversers, List<Traverser<E>> result) {
        E value = projectTraversers(previousTraversers);
        Traverser<E> traverser = new DefaultTraverser<>(value);
        result.add(traverser);
    }

    protected abstract E projectTraversers(List<Traverser<S>> previousTraversers);
